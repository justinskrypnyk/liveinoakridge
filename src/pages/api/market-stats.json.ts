// Public read of the weekly market-stats snapshot (see
...
export const GET: APIRoute = async () => {
  // Two independent sources, fetched in parallel: the weekly active-listing
  // snapshot (Blobs) and the twice-monthly sold-stats snapshot (Supabase
  // market_map_snapshots — see getSoldStatsByArea, still on the heat map's
  // cadence). Either can be empty (no data yet) without blocking the other
  // from rendering its row.
  ...
  return new Response(JSON.stringify(payload), {
    status: 200,
    // Snapshots change at most weekly — a short cache is just to avoid
